                        ...
                        <div>
                            ...
                            <select id="doctorSelect"
                                ...
                                @foreach ($doctors as $item)
                                    <option value="{{ $item->id }}" data-gender="{{ $item->gender }}"
                                        data-language="{{ $item->language }}"
                                        data-specialization="{{ $item->specialization }}"
                                        data-subspecialization="{{ $item->subSpecialization }}">
                                        {{ $item->name }}
                                    </option>
                                @endforeach
                            </select>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            doctorSelect.addEventListener('change', function() {
                ...
                // Clear old options
                genderSelect.innerHTML = '<option>Select a Gender</option>';
                languageSelect.innerHTML = '<option>Select a Language</option>';
                specializationSelect.innerHTML = '<option>Select a Specialization</option>';
                subspecializationSelect.innerHTML = '<option>Select a Sub-specialization</option>';

                if (this.value) {
                    // Populate fields based on selected doctor
                    ...
                    // ✅ Update anchor link dynamically
                    const doctorId = this.value;
                    const routeTemplate = "{{ route('find-a-doctor.singlepage', ['id' => ':id']) }}";
                    const route = routeTemplate.replace(':id', doctorId);
                    searchDoctorBtn.href = route;
                } else {
                    // If no doctor selected, reset link
